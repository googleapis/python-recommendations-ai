...
class CatalogServiceClient(metaclass=CatalogServiceClientMeta):
    ...
    @staticmethod
    def catalog_item_path_path(project: str,location: str,catalog: str,) -> str:
        """Return a fully-qualified catalog_item_path string."""
        return "projects/{project}/locations/{location}/catalogs/{catalog}/catalogItems/{catalog_item_path=**}".format(project=project, location=location, catalog=catalog, )

    @staticmethod
    def parse_catalog_item_path_path(path: str) -> Dict[str,str]:
        """Parse a catalog_item_path path into its component segments."""
        m = re.match(r"^projects/(?P<project>.+?)/locations/(?P<location>.+?)/catalogs/(?P<catalog>.+?)/catalogItems/{catalog_item_path=**}$", path)
        return m.groupdict() if m else {}
