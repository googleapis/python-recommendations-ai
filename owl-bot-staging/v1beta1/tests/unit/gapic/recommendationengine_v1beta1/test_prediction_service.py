...
def test_predict_pager(transport_name: str = "grpc"):
    client = PredictionServiceClient(
        credentials=ga_credentials.AnonymousCredentials,
    ...
    with mock.patch.object(
        ...
        assert all(isinstance(i, prediction_service.PredictResponse.PredictionResult)
                   for i in results)
def test_predict_pages(transport_name: str = "grpc"):
    client = PredictionServiceClient(
        credentials=ga_credentials.AnonymousCredentials,
        transport=transport_name,
    )
...
async def test_predict_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.predict(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_predict_rest(request_type):
    ...
    request_init = {'name': 'projects/sample1/locations/sample2/catalogs/sample3/eventStores/sample4/placements/sample5'}
...
def test_predict_rest_unset_required_fields():
    ...
    assert set(unset_fields) == (set(()) & set(("name", "userEvent", )))
...
def test_predict_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        req.return_value._content = prediction_service.PredictResponse.to_json(prediction_service.PredictResponse())
...
def test_predict_rest_bad_request(transport: str = 'rest', request_type=prediction_service.PredictRequest):
    client = PredictionServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
...
def test_predict_rest_flattened():
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        sample_request = {'name': 'projects/sample1/locations/sample2/catalogs/sample3/eventStores/sample4/placements/sample5'}
        ...
        assert path_template.validate("%s/v1beta1/{name=projects/*/locations/*/catalogs/*/eventStores/*/placements/*}:predict" % client.transport._host, args[1])
...
def test_predict_rest_pager(transport: str = 'rest'):
    ...
    with mock.patch.object(Session, 'request') as req:
        ...
        response = tuple(prediction_service.PredictResponse.to_json(x) for x in response)
...
def test_prediction_service_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.cloud.recommendationengine_v1beta1.services.prediction_service.transports.PredictionServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_prediction_service_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.cloud.recommendationengine_v1beta1.services.prediction_service.transports.PredictionServiceTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.PredictionServiceTransport()
        adc.assert_called_once()
